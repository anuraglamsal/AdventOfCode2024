...
def check_loop(_grid, _dy, _dx, _guard_pos_y, _guard_pos_x, _dir_index):
    _grid_visited = []
    while True:
        if (_guard_pos_y, _guard_pos_x, _dir_index) not in _grid_visited:
            _grid_visited.append((_guard_pos_y, _guard_pos_x, _dir_index))
        else:
            global count
            count += 1
            return
...
print(f"{(end-start)/60} minutes taken to get the answer.") # 2.36 mins on my computer. 
